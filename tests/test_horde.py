...
class TestHorde:
    ...
    def setupHorde(self, h, width, height):
        ...
        base = h.h3dAddNode(pyhorde3d.h3dRootNode, res_loaded['base'])
        h.h3dSetNodeTransform(base, 0, 1000, 0, 0, 0, 0, 1, 1, 1)
        del base

        # Add the skybox to the scene and scale it.
        s = int(0.9 * 5000 / 1.7)
        skybox = h.h3dAddNode(pyhorde3d.h3dRootNode, res_loaded['sky'])
        ...
        cam = h.h3dAddCameraNode(pyhorde3d.h3dRootNode, 'Camera', res_loaded['shader'])
        near, far = 0.1, 5000
        ...
        h.h3dSetNodeParamI(cam, pyhorde3d.h3dCamera.ViewportXI, 0)
        h.h3dSetNodeParamI(cam, pyhorde3d.h3dCamera.ViewportYI, 0)
        h.h3dSetNodeParamI(cam, pyhorde3d.h3dCamera.ViewportWidthI, width)
        h.h3dSetNodeParamI(cam, pyhorde3d.h3dCamera.ViewportHeightI, height)
        h.h3dSetNodeParamI(cam, pyhorde3d.h3dCamera.OccCullingI, 0)
    ...
    def test_create_image(self):
        ...
        width = height = 512
        eglDpy = pyhorde3d.createEGLContext(width, height)
        h = pyhorde3d.PyHorde3D(width, height, GLVersion=2)

        # Create screenshot and release EGL context.
        assert not os.path.exists(fname)
        h.h3dScreenshotFile(fname)
        assert os.path.exists(fname)
        pyhorde3d.releaseEGLContext(eglDpy)
